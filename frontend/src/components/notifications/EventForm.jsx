...
const DEFAULT_FORM = { eventName: '', date: '', time: '', venue: '' };
...
export default function EventForm() {
  ...
  const allFilled = form.eventName && form.date && form.time && form.venue;

  const handleSend = async () => {
    if (!allFilled) return toast.error('Fill all event details');
    if (recipients.length === 0) return toast.error('Add at least one recipient');
    setSending(true);
    setResult(null);
    try {
      const res = await notificationAPI.sendEvent({ recipients, ...form });
  ...
  return (
    <div className="space-y-6">
      ...
      {preview && recipients.length > 0 && (
        <div className="border border-gray-200 rounded-lg overflow-hidden text-sm">
          ...
          <div className="p-4 whitespace-pre-wrap text-gray-700 leading-relaxed">
            {`Dear ${recipients[0]?.name}, you are cordially invited to *${form.eventName || '[Event Name]'}* on ${form.date || '[Date]'} at ${form.time || '[Time]'}.\nVenue: ${form.venue || '[Venue]'}\nWe look forward to your presence.\n— [Institution Name]`}
          </div>
      ...
      >
        {sending ? `Sending... (${recipients.length} recipients)` : `Send Event Invitation`}
      </button>
    </div>
  );
}
